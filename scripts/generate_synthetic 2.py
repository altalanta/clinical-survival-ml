"""Generate synthetic right-censored survival dataset for testing."""
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=Path("data/toy/toy_survival.csv"))
    parser.add_argument("--n", type=int, default=400)
    parser.add_argument("--seed", type=int, default=42)
    args = parser.parse_args()

    df, meta = simulate_survival(args.n, args.seed)
    args.out.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(args.out, index=False)
    print(f"Wrote {len(df)} rows to {args.out} (seed={meta['seed']})")
