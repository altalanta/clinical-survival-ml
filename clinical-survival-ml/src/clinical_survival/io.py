...
def load_dataset(
    ...
    *,
    time_col: str = "time",
    event_col: str = "event",
    external_config: Optional[Dict[str, object]] = None,
) -> Tuple[Tuple[pd.DataFrame, pd.DataFrame], Optional[Tuple[pd.DataFrame, pd.DataFrame]], Dict[str, object]]:
    ...
    metadata["n_samples"] = int(len(train_df))
    metadata["external_present"] = external_split is not None
    metadata["n_external_samples"] = int(external_count)
